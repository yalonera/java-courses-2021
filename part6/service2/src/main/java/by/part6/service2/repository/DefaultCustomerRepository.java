...
import by.part6.service.model.AddressInfo;
...
public class DefaultCustomerRepository implements CustomerRepository {
  ...
  private final List<Product> generatedProducts = findProducts();
  private final List<PaymentInfo> generatedPaymentInfos = findPaymentInfos();

  @Override
  public List<Product> findProducts() {
    return generate(productNames.size(), this::createProduct);
  }
  ...
  @Override
  public List<Order> findOrders(final int count) {
    return generate(count, this::createOrder);
  }

  @Override
  public List<PaymentInfo> findPaymentInfos() {
    return generate(cardNumbers.size(), this::createPaymentInfo);
  }

  @Override
  public List<OrderItem> findOrderItems(final int count) {
    return generate(count, this::createOrderItem);
  }
  ...
  private Customer createCustomer(final int i) {
    ...
    customer.setOrders(findOrders(randomInt(MIN_ORDER_COUNT, MAX_ORDER_COUNT)));
    return customer;
  }
  ...
  private List<OrderItem> getDistinctOrderItems() {
    return findOrderItems(randomInt(MIN_ORDER_ITEMS, MAX_ORDER_ITEMS))
        .stream()
  ...
  private Product createProduct(final int i) {
    final int idx = i % productNames.size();
    final Product product = new Product();
    productId += (long) idx * PRODUCT_ID_INCREMENT;
    product.setId(productId);
    product.setName(productNames.get(idx));
    product.setColor(Product.Color.values()[idx % Product.Color.values().length]);
    product.setPrice(BigDecimal.valueOf((double) randomInt(MIN_PRODUCT_PRICE, MAX_PRODUCT_PRICE) / 100));
    return product;
  }
  ...
  private PaymentInfo createPaymentInfo(final int i) {
  ...
  private <T> List<T> generate(final int count, final Supplier<T> supplier) {
