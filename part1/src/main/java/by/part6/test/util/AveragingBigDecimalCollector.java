...
public class AveragingBigDecimalCollector implements Collector<OrderItem, BigDecimal[], BigDecimal> {
    ...
    public BiConsumer<BigDecimal[], OrderItem> accumulator() {
        return (acc, b) -> {
            acc[0] = acc[0].add(b.getProduct().getPrice().multiply(new BigDecimal(b.getQuantity())));
            acc[1] = acc[1].add(new BigDecimal(b.getQuantity()));
        };
    }

    @Override
    public BinaryOperator<BigDecimal[]> combiner() {
        return (acc1, acc2) -> new BigDecimal[]{acc1[0].add(acc2[0]), acc1[1].add(acc2[1])};
    }
